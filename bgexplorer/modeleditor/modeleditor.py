...
class ModelEditor(object):
    ...
    def onregister(self, setupstate):
        ...
        if 'bootstrap' not in setupstate.app.blueprints:
    ...
    def newmodel(self):
        ...
        if importfile:
            ...
            newmodel = self.modeldb.new_model(
                derivedFrom, name=name, simsdb=simsdbview)
            newid = newmodel.id
    ...
    def delcomponent(self, modelid, componentid):
        model = getmodelordie(modelid, toedit=True)
        getcomponentordie(model, componentid)
